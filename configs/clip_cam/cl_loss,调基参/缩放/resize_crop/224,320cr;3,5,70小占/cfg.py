...
cfg._cfgs_update_at_parser = Param2Tune([('configs/clip_cam/cl_loss,调基参/base.py',
                                          'configs/patterns/aug/voc_resize_crop.py',) + c
                                         for c in
                                         [(),
                                          ('configs/clip_cam/cl_loss,调基参/_patches/3.8k,1e-4小lr.py',)]])

# * 实验不同scale策略。
cfg.auger.train.ini.scale_crop_method.size = Param2Tune([224, 320])
cfg.auger.train.ini.scale_crop_method.scale = Param2Tune([(0.3, 1.0), (0.5, 1.0), (0.7, 1.0)])
